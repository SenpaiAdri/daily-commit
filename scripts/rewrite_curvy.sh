...
# Allow overriding end date via REWRITE_END (epoch seconds). Default: now.
if [[ -n "${REWRITE_END:-}" ]]; then
  end_ts=$REWRITE_END
else
  end_ts=$(date -u +%s)
fi
...
synced=0
day_ts=$start_ts
# 14-day smooth cycle to create a curvier pattern
cycle=(1 2 2 3 3 4 3 3 2 2 1 1 2 3)
while [[ $day_ts -le $end_ts ]]; do
  day=$(date -u -d "@${day_ts}" +%F)
  checksum=$(printf "%s" "$day" | cksum | awk '{print $1}')
  weekday=$(date -u -d "@${day_ts}" +%u)  # 1=Mon .. 7=Sun
  doy=$(date -u -d "@${day_ts}" +%j)
  offset=$(( checksum % 14 ))
  idx=$(( (10#$doy + offset) % 14 ))
  target=${cycle[$idx]}
  # Weekend bias: lift Sat/Sun by +1, cap at 4
  if [[ $weekday -ge 6 ]]; then
    target=$((target + 1))
  fi
  if [[ $target -gt 4 ]]; then target=4; fi
  ...
  if [[ $synced -eq 0 ]]; then
    ...
    git add -A
    # Disable the workflow job to prevent new commits while we finalize history
    if [[ -f ".github/workflows/daily_commit.yml" ]]; then
      sed -i '/^  update-file:$/a\    if: ${{ false }}' .github/workflows/daily_commit.yml || true
      git add .github/workflows/daily_commit.yml || true
    fi
